...
"use client";
import React, { useEffect, useRef, useState } from "react";
import { X, Trash2, Printer, CheckCircle, Bell, User  } from "lucide-react";
import Swal from "sweetalert2";
       ...
       <div className="flex justify-between items-center mb-8 p-5 rounded-xl bg-white shadow-sm border">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            ...
            {showNotifications && pendingOrdersCount > 0 && (
              <div className="absolute right-0 mt-3 w-56 bg-white shadow-xl border rounded-lg z-50">
                ...
                {todayOrders.filter((o) => o.status === "Pending").map((o) => (
                  <p key={o.order_id} className="p-2 text-sm border-b">
                    Table: {o.tableName} ({getTableLocation(o.table_id)})
                  </p>
                ))}
          ...
          <div className="relative">
            <button
              ref={profileRef}
              className="p-3 bg-amber-50 hover:bg-amber-100 rounded-full shadow-sm"
              onClick={() => setShowProfileDropdown(!showProfileDropdown)}
            >
              <User className="w-6 h-6 text-amber-700" />
            </button>
            {showProfileDropdown && (
              <div className="absolute right-0 mt-3 w-40 bg-white shadow-xl border rounded-lg z-50">
                <button className="w-full text-left p-3 hover:bg-gray-100">Profile</button>
                <button
